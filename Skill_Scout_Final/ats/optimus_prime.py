...
RESULTS_FOLDER = 'D:/New folder/Skill_Scout_Final/ats/matched_candidates'
...
def search_applicants(skills_needed, threshold):
    ...
    for filename in os.listdir('D:/New folder/Skill_Scout_Final/ats/resumes'):
        filepath = os.path.join('resumes', filename)
        text = extract_text(filepath)
        applicant_skills = set()
        for skill_key, skill_values in SKILLS.items():
            for skill_value in skill_values:
                if skill_value.lower() in text:
                    applicant_skills.add(skill_key)
                    break
        
        if len(applicant_skills) / len(skills_needed) >= threshold:
            num_matched_skills += len(applicant_skills)
            matched_skills.append((filename, extract_email(filepath), applicant_skills))
